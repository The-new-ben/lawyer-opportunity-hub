-- Insert successful closed cases with judge oversight and ratings
...
-- Add high ratings for these successful cases
...
SELECT 
  ...
  CASE rn 
    ...
    WHEN 6 THEN 4 WHEN 7 THEN 5 WHEN 8 THEN 5 WHEN 9 THEN 4 WHEN 10 THEN 5
  END as score,
  CASE rn 
    WHEN 1 THEN 'עורכת דין מצוינת! טיפלה בתיק בצורה מקצועית ביותר. התוצאה היתה מעבר לציפיות.'
    WHEN 2 THEN 'מומחיות יוצאת דופן בעסקאות מסחריות. ליווי מקצועי לאורך כל התהליך.'
    WHEN 3 THEN 'ניהלה את התביעה הייצוגית ברמה הגבוהה ביותר. מסורה ומקצועית.'
    WHEN 4 THEN 'עורך דין שמבין את העניין ולוחם על הצדק. המלצה בחום!'
    WHEN 5 THEN 'הובילה עסקה מורכבת בצורה מושלמת. תוצאה מעל הציפיות.'
    WHEN 6 THEN 'מקצועי ומנוסה. הגיע להסכם מצוין למען העובדים.'
    WHEN 7 THEN 'עזרה להקים את החברה בצורה מושלמת. ייעוץ משפטי ברמה הגבוהה ביותר.'
    WHEN 8 THEN 'לוחם אמיתי על זכויות הפצועים. ניסיון עשיר ותוצאות מצוינות.'
    WHEN 9 THEN 'מנהלת עסקאות נדלן בצורה מקצועית ויעילה.'
    WHEN 10 THEN 'מומחה אמיתי בדיני עבודה. פתר מחלוקת מורכבת בדרך יצירתית.'
  END as comment
FROM case_ratings;