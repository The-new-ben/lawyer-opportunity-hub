...
-- Create contracts table for digital contract management
...
CREATE POLICY "Lawyers and admins can manage contracts" 
...
-- Update leads table with new status values
ALTER TABLE public.leads ALTER COLUMN status TYPE TEXT;
UPDATE public.leads SET status = 'new' WHERE status IS NULL;

-- Update cases table with new status values  
ALTER TABLE public.cases ALTER COLUMN status TYPE TEXT;
